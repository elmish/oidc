...
let tests = testList "Token" [
    ...
    testList "validateClaims" [
        ...
        testCase "accepts valid claims" <| fun _ ->
            ...
            let payload = validPayload ()
            let result = validateClaims testOptions (Some "test-nonce") now header payload
        ...
        testCase "rejects algorithm 'none' (algorithm confusion attack)" <| fun _ ->
            ...
            let payload = validPayload ()
            let result = validateClaims testOptions (Some "test-nonce") (nowEpoch ()) header payload
            match result with
        ...
        testCase "rejects issuer mismatch" <| fun _ ->
            ...
            let result = validateClaims testOptions (Some "test-nonce") (nowEpoch ()) validHeader payload
            match result with
        ...
        testCase "normalizes issuer trailing slash" <| fun _ ->
            ...
            let payload = validPayload ()
            let result = validateClaims opts (Some "test-nonce") (nowEpoch ()) validHeader payload
        ...
        testCase "rejects expired token beyond clock skew" <| fun _ ->
            ...
            let result = validateClaims testOptions None now validHeader payload
            match result with
        ...
        testCase "rejects future-dated iat beyond clock skew" <| fun _ ->
            ...
            let result = validateClaims testOptions (Some "test-nonce") now validHeader payload
            match result with
        ...
        testCase "rejects nonce mismatch" <| fun _ ->
            ...
            let result = validateClaims testOptions (Some "expected-nonce") (nowEpoch ()) validHeader payload
            match result with
        ...
        testCase "accepts missing nonce when not required" <| fun _ ->
            let payload = { validPayload () with nonce = None }
            let result = validateClaims testOptions None (nowEpoch ()) validHeader payload
            Expect.isOk result "no nonce required -> should accept"

        testCase "checks run in priority order (alg before issuer)" <| fun _ ->
            let header = { alg = "none"; kid = "test-kid-1" }
            let payload = { validPayload () with iss = "https://evil.example.com" }
            let result = validateClaims testOptions None (nowEpoch ()) header payload
            match result with
