...
let tests = testList "Renewal" [
    ...
    testList "DotNetRenewal.refreshToken" [
        ...
        testCaseAsync "exchanges refresh token and validates new id_token" <|
            async {
                ...
                let newIdToken = signJwt rsa headerJson payloadJson
    ...
    testList "expirySubscription" [
        testCase "creates disposable interval" <| fun _ ->
            ...
            let sub = Renewal.expirySubscription DotNet.timer dispatch
            // Just verify it returns a disposable without error
            sub.Dispose()
    ]
]
